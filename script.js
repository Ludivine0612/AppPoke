// ==========================================
// 0. FONCTION DE NETTOYAGE (ANTI-ERREUR ENFANTS)
// ==========================================
function simplifierTexte(texte) {
    ...
    return texte
        ...
        .replace(/[^a-z0-9]/g, "")                 // Enlève tirets, espaces cachés et caractères spéciaux
...
async function chargerDonnees() {
    try {
        ...
        if (resFr && resFr.ok) {
            ...
            dataFr.forEach(p => {
                let nomNettoye = simplifierTexte(p.name.fr);
            ...
            // On simplifie aussi les clés du dictionnaire de secours au chargement
            for (let keyId in backupFr) {
                let cleSimplifiee = simplifierTexte(backupFr[keyId]);
                pokedexTraduction[cleSimplifiee] = parseInt(keyId);
            }
        }
...
// 3. RECUPERER UN POKÉMON (RECHERCHE ENTIÈREMENT NETTOYÉE ET FLEXIBLE)
async function getPokemon() {
    ...
    // SÉCURITÉ ANTI-BUG : Si le champ est vide
    if (!input || !input.value || input.value.trim() === "") {
        return; 
    }

    // 1. On récupère la saisie brute
    let rawSearch = input.value.trim();

    // 2. Nettoyage magique unifié (sans accents, sans majuscules, sans espaces ou tirets parasites)
    let cleanDicoKey = simplifierTexte(rawSearch);
    
    // Pour l'API de secours (on remplace par des tirets au cas où)
    let apiSearchValue = rawSearch.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim().replace(/\s+/g, "-");
    ...
    try {
        ...
        // B. Si le nom est trouvé dans ton dictionnaire local mis à plat
        else if (typeof pokedexTraduction !== 'undefined' && pokedexTraduction[cleanDicoKey]) {
            id = pokedexTraduction[cleanDicoKey];
        } 
        // C. Traduction en direct via l'API Species en dernier recours
        else {
            try {
        ...
        // Nettoyage et affichage des types
        if (typesContainer) {
        ...
        // Nettoyage et affichage des statistiques
        if (statsContainer) {
        ...
        // Ajout au Pokédex (Grille principale)
...
function updateGrid(id, spriteUrl, name) {
    ...
    if (progressionParRegion[range] && !progressionParRegion[range].has(id)) {
        progressionParRegion[range].add(id); 
...
// 5. MODALES ET SAUVEGARDE COMPLÈTE
...
async function lancerDuel(playerId) {
    ...
    try {
        ...
        if (forceJoueur > defenseAdversaire) {
            ...
            log.innerHTML = `<span style="font-weight:bold; color: #ffde00;">MATCH NUL !</span><br>Votre puissance is identique (${forceJoueur}).`;
        }
